...
class Option
{
    ...
    //         'key' => 'hacoidev/nguonc-crawler.options',
    //     ], [
    //         'name' => 'Options',
    ...
    public static function getAllOptions()
    {
        ...
        try {
            $categories = Cache::remember('nguonc_categories', 86400, function () {
                $data = json_decode(file_get_contents(sprintf('%s/the-loai', config('nguonc_crawler.domain', 'https://nguonc1.com'))), true) ?? [];
                return collect($data)->pluck('name', 'name')->toArray();
            });

            $regions = Cache::remember('nguonc_regions', 86400, function () {
                $data = json_decode(file_get_contents(sprintf('%s/quoc-gia', config('nguonc_crawler.domain', 'https://nguonc1.com'))), true) ?? [];
                return collect($data)->pluck('name', 'name')->toArray();
            });
        } catch (\Throwable $th) {
